...
export function generateFloorPlan(start: boolean): ROOM_TYPE[][] {
    ...
    for (let row = 0; row < 5; row++) { // Fill each tile with NULL (-1)
    ...
    if (start) { // Start at middle bottom, Boss at top row
        floor[4][2] = ROOM_TYPE.START; // Initialize Start
        startCoord = [4, 2];
        floor[0][bossCol] = ROOM_TYPE.BOSS; // Initialize Boss
    } else { // Start at the middle in middle row, Boss at side columns
        // Initialize Start
        floor[2][2] = ROOM_TYPE.START;
        startCoord = [2, 2];

        // Initialize Boss
        while (bossRow == 2) { // If Boss room is directly two tiles to the left or right of Start, rng again
            bossRow = Math.floor(Math.random() * 5);
        }

        if (stage2Col == 0) { // Left side
    ...
    if (start) { // Stage 1 or 3
        goals.push([0, bossCol])
    } else { // Stage 2
        if (stage2Col == 0) { // Left column
    ...
    for (const goal of goals) {
        ...
        while (floor[current[0]][current[1]] !== ROOM_TYPE.START) { // While current room is not Start room
            // pathway.push(current); // Push current coordinate to pathway list
            if ( // Ignore if current room is Boss, Mini Boss or Shop rooms
